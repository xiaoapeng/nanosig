...
static int test_invalid_args_and_lifecycle(void)
{
    int initialized = 1;
    ns_loop_t *loop = NULL;
    const ns_loop_t *dummy_loop = (const ns_loop_t *)(uintptr_t)1u;
    int is_owner = 0;

    EXPECT_OK(ns_is_initialized(NULL) == NS_E_INVAL);
    EXPECT_OK(ns_loop_quit(NULL) == NS_E_INVAL);
    EXPECT_OK(ns_loop_current(NULL) == NS_E_INVAL);
    EXPECT_OK(ns_loop_is_owner(NULL, &is_owner) == NS_E_INVAL);
    EXPECT_OK(ns_loop_is_owner(dummy_loop, NULL) == NS_E_INVAL);

    EXPECT_OK(ns_is_initialized(&initialized) == NS_OK);
    EXPECT_OK(initialized == 0);
    EXPECT_OK(ns_loop_create(&loop, NULL) == NS_E_SHUTDOWN);
    EXPECT_OK(ns_loop_current(&loop) == NS_E_SHUTDOWN);
    ...
    EXPECT_OK(ns_loop_current(&loop) == NS_E_NO_LOOP);
    EXPECT_OK(ns_shutdown() == NS_OK);
    EXPECT_OK(ns_is_initialized(&initialized) == NS_OK);
    EXPECT_OK(initialized == 0);
    EXPECT_OK(ns_shutdown() == NS_OK);
...
static int test_same_thread_binding(void)
{
    ...
    EXPECT_OK(ns_loop_current(&current) == NS_E_NO_LOOP);
    EXPECT_OK(ns_loop_create(&loop, &cfg) == NS_OK);
    EXPECT_OK(loop != NULL);
    EXPECT_OK(ns_loop_current(&current) == NS_OK);
    EXPECT_OK(current == loop);
    EXPECT_OK(ns_loop_is_owner(loop, &is_owner) == NS_OK);
    EXPECT_OK(is_owner != 0);
    EXPECT_OK(ns_loop_create(&second, &cfg) == NS_E_EXISTS);
    EXPECT_OK(ns_shutdown() == NS_E_EXISTS);
...
static void loop_thread_run(loop_thread_ctx_t *ctx)
{
    ...
    rc = ns_loop_current(&current);
    if((rc != NS_OK) || (current != ctx->loop)){
        rc = NS_E_INVAL;
        ns_atomic_store_explicit(&ctx->failure_line, __LINE__, ns_memory_order_relaxed);
        goto out;
    }
    rc = ns_loop_is_owner(ctx->loop, &is_owner);
    if((rc != NS_OK) || (is_owner == 0)){
        rc = NS_E_INVAL;
        ns_atomic_store_explicit(&ctx->failure_line, __LINE__, ns_memory_order_relaxed);
        goto out;
    }
...
out:
    if(ctx->loop != NULL){
        int destroy_rc = ns_loop_destroy();
...
static int test_cross_thread_quit_and_ownership(void)
{
    ...
    EXPECT_OK(ns_loop_current(&current) == NS_E_NO_LOOP);
    EXPECT_OK(ns_loop_is_owner(ctx.loop, &is_owner) == NS_OK);
    EXPECT_OK(is_owner == 0);
    EXPECT_OK(ns_loop_destroy() == NS_E_NO_LOOP);
